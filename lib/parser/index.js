...
export function parseMarkdown(src) {
    ...
    const elements = mdast.children
        .map(convertBlock)
        .filter(Boolean);
    
    return file(program([expressionStatement(arrayExpression(elements))]));
}
...
function convertBlock(node) {
    ...
    return callExpression(identifier('raw'), [stringLiteral(node.type)]);
}
